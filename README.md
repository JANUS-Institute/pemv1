...
### Using `uv`
Create a virtual environment and activate it. Note that on windows the activation command may be different. Follow the instructions printed by `uv venv` to get the correct command.
```
uv venv
...
By default, this is `0.18.7`:, which was the version used in the paper.
```
...
That folder in turn contains folders (numbered 000000 - num_samples) that hold the model output json files for each sample generated during MCMC.
These can be used to plot results.
...
At present, these are slightly incompatible with the present version of hallmd and pem_core, owing to a refactor of those repos.
Many of the individual functions are still useful, however, and we hope to update these in the near future.
