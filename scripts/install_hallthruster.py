...
def install_juliaup(yes: bool = False, verbose: bool = True):
    ...
    if PLATFORM == "windows":
        ...
        ENV["PATH"] = (
            str((Path(os.path.expanduser("~")) / ".juliaup" / "bin").resolve())
            + os.pathsep
            + ENV.get("PATH", "")
        )
...
def ensure_julia_version(julia_version, verbose):
    ...
    try:
        proc_ret = run_command(
            "juliaup status", text=True, capture_output=True, env=ENV
        )
    ...
    for line in cmd_output.splitlines():
        if "Channel" not in line:
            ...
            ]  # [Default, Channel, Version, Update] columns
            if len(parts) > 1:
                for idx, p in enumerate(parts):
                    ...
                    char = (
                        "+" if "+" in p else ("-" if "-" in p else "")
                    )  # Version column has "+" or "-" for installed
                    if char:
    ...
    if found_installed:
        ...
        print(
            f"No suitable version found. Installing and setting Julia version {julia_version} as default."
        )
        run_command(
            f"juliaup add {julia_version}", capture_output=False, text=True, env=ENV
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Install specified Julia and HallThruster.jl versions."
    )
    parser.add_argument(
        "-j",
        "--julia-version",
