...
import pem_core.distribution as distributions
import pem_core.transform as transform
...
def analyze(
    ...
    device = hallmd.utils.load_thruster(device_name)
    component_names = set([c.name for c in system.components])

    variables, samples, logposts, accepted, ids = sampling.read_output_file(logfile)
    ...
    if plot_bands or calc_metrics:
        ...
        if secondary_simulation is not None:
            ...
            _, _, _, _, ids_2 = sampling.read_output_file(secondary_dir / "mcmc.csv")
